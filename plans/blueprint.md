...
| Catalog GC of dropped rows | Physical `delete(key)` during background GC | Refuse by default; gated behind the explicit, audited `rocklake excise` command |
| `retain-from` advancement | Silently delete rows below the floor | Advance the `retain-from` key transactionally (visibility only, no byte deletion); separate `rocklake excise` command handles physical deletion |
...
Both go through `rocklake excise`:

- Default physical retention is **infinite**. `rocklake gc` only advances
  `retain-from` (query-visibility floor); it never calls `object_store.delete`.
- Physical deletion requires `rocklake excise --before <snapshot> --apply`,
...
primitives in `rocklake-core` (key encoding, SDKV value header, counter
...
logic) stay in `rocklake-catalog`. For the concrete extraction boundary, the
...
├── crates/
│   ├── rocklake-core/       # Catalog model: types, key encoding,
│   │                         #   SlateDB read/write primitives
│   ├── rocklake-catalog/    # DuckLake-spec-aware operations
│   │                         #   (snapshot, schema, table, file, stats)
│   ├── rocklake-sql/        # Minimal SQL parser/executor for the
│   │                         #   spec query set (used by strategies B/C)
│   ├── rocklake-sqlite-vfs/ # Optional Strategy A spike only
│   ├── rocklake-pgwire/     # Strategy B: PG-wire sidecar binary
│   └── rocklake-ffi/        # C ABI for strategy C
...
| Concurrent `open_or_create` cannot be made race-free | Require explicit `rocklake init` under a deployment lock before serving DuckDB clients. |
...
### Phase 1 — Catalog data model in SlateDB (`rocklake-core`)
...
#### 1.5 Module surface (`rocklake-core`)
...
### Phase 2 — DuckLake-spec operations (`rocklake-catalog`)
...
### Phase 4 — Strategy B: PostgreSQL-wire sidecar (`rocklake-pgwire`)
...
- [`pgwire`](https://crates.io/crates/pgwire) crate for the wire
  protocol.
- A **minimal SQL frontend** (`rocklake-sql`) that recognises *only*
...
- Statement planner → `rocklake-catalog` operations.
...
examples below. The `rocklake-sql` frontend only needs to recognise and
...
pattern-matching on the AST to dispatch to `rocklake-catalog` methods keeps
...
**Deliverable:** A `rocklake serve` binary that exposes a SlateDB
...
**Deliverable:** `INSTALL rocklake; ATTACH 'ducklake:slatedb://…' AS
...
- **Catalog visibility GC and excision.** DuckLake's `DROP TABLE` only
  ...
  (1) `rocklake gc` advances the `retain-from` visibility floor (no
  physical deletion); (2) `rocklake excise` physically removes catalog
...
- **Checkpoints / backups.** Expose `rocklake checkpoint create` and
  `rocklake checkpoint restore` thin wrappers around SlateDB's
...
- **Catalog validation and repair.** Provide `rocklake inspect`,
  `rocklake verify`, and `rocklake repair --dry-run` commands before
...
- **Embed in non-DuckDB engines.** Expose `rocklake-catalog` to
  DataFusion's
...
`rocklake-catalog` that runs this check on demand and as part of the
periodic GC cycle.
...
Before implementing `rocklake-sql` from scratch, evaluate
  ...
  └── pgwire crate (PostgreSQL wire protocol)
       └── GlueSQL (SQL parsing + execution)
            └── Store / StoreMut trait implementations
                 └── rocklake-catalog (typed Rust API)
...
Add a basic `rocklake verify catalog` command during Phase 2, not Phase 6.
...
`rocklake-catalog` must accept a `DuckLakeType` argument alongside the
...
**Implementation note:** Define the `CatalogPath` struct in `rocklake-core`
...
2. **Excision (rare, audited).** Physical deletion of historical facts. This
   ...
   `rocklake excise` command described in section 1.4 and section 5.28. It is
...
SlateDB key.** Physical deletion only happens via `rocklake excise` and only
...
fact.end_snapshot IS NOT NULL
  ...
  AND operator explicitly invokes `rocklake excise --apply`
```
...
5.2. The default `rocklake gc` command never runs the excision step.
...
storage. Only `rocklake excise --apply` may delete Parquet files, and only
...
`--retention-days N`, and `rocklake gc` never deletes bytes. This means time
...
`rocklake excise --before <snapshot> --apply` on a schedule with a recorded
...
path (`rocklake gc plan` / `rocklake gc apply` for visibility,
`rocklake excise plan` / `rocklake excise apply` for physical deletion) is
...
```rust
#[no_mangle]
pub extern "C" fn rocklake_get_snapshot(catalog: *mut Catalog, id: u64) -> ... {
...
- **Physical deletes in excision.** When `rocklake excise` runs, it issues
...
(`rocklake-sql`) will receive SQL containing `$1`, `$2`, etc. rather than
literal values, and must substitute parameter values at execution time.

**This affects the pattern matcher in `rocklake-sql`.** A classifier that
...
a single file `crates/rocklake-core/src/tags.rs` that lists all 28 tables
...
`rocklake verify catalog` subset is a Phase 2 deliverable; the data-file
verification and repair commands can arrive during Phase 6 hardening.

| Command | Purpose |
| --- | --- |
| `rocklake inspect snapshot --latest` | Print current snapshot, schema version, counters, file counts |
| `rocklake verify catalog` | Check primary-key uniqueness, foreign keys, MVCC intervals, counter monotonicity |
| `rocklake verify data-files` | `HEAD` every referenced Parquet/delete file, optionally sample footers |
| `rocklake gc plan` | Show what catalog rows and data files would be deleted under current retention |
| `rocklake repair --dry-run` | Propose repairs for orphaned files, dangling references, or counter drift without mutating state |
...
#### Extraction boundary: `rocklake-core` vs `rocklake-catalog`

When `rocklake-factstore` is carved out for v2.x the split follows the line
between generic storage primitives and DuckLake-specific application logic:

| Stays in `rocklake-catalog` (DuckLake-specific) | Moves to `rocklake-factstore` (reusable) |
| --- | --- |
...
`rocklake-core` already uses a neutral `SnapshotId(u64)` newtype in its
public API. Keep DuckLake-specific naming (`dl_snapshot_id`, `catalog_version`)
as variable-name conventions within `rocklake-catalog`; never bake them into
...
a self-contained `Db`; `rocklake-factstore` is a packaging of the generic
primitives, and a new schema implementation opens its own `Db` path.

#### C ABI versioning for Strategy C (`rocklake-ffi`)
...
- `rocklake-ffi` exports `uint32_t rocklake_abi_version()` returning a
  compile-time constant encoded as `major * 1000 + minor` (e.g. `1003` for
  v1.3).
- The DuckDB extension calls `rocklake_abi_version()` at load time and
...
0xFF | "writer-endpoint" → "pod-a.rocklake.svc:5432"
```
...
| --- | --- | --- |
| `rocklake-writer` / `rocklake-reader` | Catalog role | SlateDB catalog prefix only |
| DuckDB ingestion/query jobs | Data role | Parquet data/delete-file prefix only |
| `rocklake gc` / maintenance job | Maintenance role | Read catalog plus delete eligible data files |
...
- Should we target the upstream `ducklake` extension or build a
  separate `rocklake` extension? Upstream is cleaner but slower
  socially.
