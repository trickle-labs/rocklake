...
│   ├── what-is-rocklake.md          # Conceptual introduction
...
│   └── what-rocklake-is-not.md      # Explicit non-goals and anti-patterns
├── performance/
...
then walks through `rocklake serve --catalog ./my-catalog --bind 0.0.0.0:5432`, shows
...
(`rocklake-factstore` crate) is where this goes, but the page should explain the
...
The page should be transparent about what is speculative. The `rocklake-factstore`
...
tag space, the separation between `rocklake-core` and `rocklake-catalog`, the strict
...
sql` parses and classifies the AST; `rocklake-catalog` executes the corresponding
read operation; `rocklake-core` does a SlateDB `scan_prefix` with an MVCC filter;
...
layering: `rocklake-core` has no workspace dependencies; `rocklake-catalog` depends
on `rocklake-core`; `rocklake-sql` depends on `rocklake-core`; `rocklake-pgwire`
depends on all three; `rocklake-ffi` re-exports `rocklake-catalog` through a C ABI;
`rocklake-sqlite-vfs` is an optional spike with no dependency on the main stack.

For each crate, the page gives the primary module structure, the key public types (e.g.,
`CatalogStore`, `CatalogReader`, `CatalogWriter` in `rocklake-catalog`; `CatalogKey`,
`SnapshotId`, `TableTag` in `rocklake-core`), and a sentence on what belongs in the
...
The `rocklake-sql` crate implements a bounded SQL classifier that recognises exactly
...
with expected output, a working `rocklake serve` invocation, a DuckDB connection
...
the directory structure that `rocklake serve --catalog ./my-catalog` creates under the
...
addresses the GC/maintenance exception: the `rocklake gc apply` and `rocklake excise`
...
The CLI reference is a complete, structured description of every `rocklake` subcommand,
...
explains the expected ranges. For example, `rocklake_snapshots_created_total` should
increase monotonically; a sudden stop indicates the writer has stalled. Object-store
throttle retries (`rocklake_object_store_throttle_retries_total`) are normal in small
...
`--retention-days 30` and schedule `rocklake gc apply` to run daily; an operator who
...
The `rocklake gc plan` command is explained in detail: it shows what the advancement
...
before running `rocklake gc apply` in production for the first time.
...
The page walks through the full excision workflow: `rocklake excise plan --before
<snapshot>` to see what would be deleted; reviewing the plan; running `rocklake excise
apply --before <snapshot> --reason "GDPR erasure request #1234" --operator alice` to
execute the deletion with a recorded audit entry; and then running `rocklake verify
...
For migrations, the page documents the three-step path: `rocklake export` the current
catalog to NDJSON, reinitialize with the new binary, `rocklake import` the NDJSON. A
...
constraint), `retain-from` (the query-visibility floor advanced by `rocklake gc apply`;
...
clear error messages and resolution steps. A CLI reference entry for `rocklake gc`
...
`rocklake serve` and `duckdb` commands with expected output. Every CLI reference entry
...
not "it is possible to query historical snapshots"; "run `rocklake verify catalog` to
check integrity" not "integrity can be verified by running `rocklake verify catalog`."
...
repo_name: trickle-labs/rocklake
repo_url: https://github.com/trickle-labs/rocklake
...
extra:
  social:
    - icon: fontawesome/brands/github
      link: https://github.com/trickle-labs/rocklake
  generator: false
  status:
    new: Recently added
...
| `crates/rocklake-core/src/tags.rs` | Tag Allocation internals, Catalog Tables reference (authoritative source) |
| `crates/rocklake-pgwire/src/` | PG-Wire Protocol architecture, SQL Dispatcher architecture |
| `crates/rocklake-catalog/src/` | Operations reference, Catalog Tables reference |
| `crates/rocklake-sql/src/` | SQL Dispatcher architecture, Supported SQL reference |
