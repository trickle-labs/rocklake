...
- `crates/rocklake-core/src/rows.rs`
- `crates/rocklake-core/src/tags.rs`
- `crates/rocklake-core/src/keys.rs`
- `crates/rocklake-core/src/types.rs`
- `crates/rocklake-catalog/src/reader.rs`
- `crates/rocklake-catalog/src/writer/mod.rs`
- `crates/rocklake-catalog/src/writer/snapshot.rs`
...
- `crates/rocklake-pgwire/src/executor/meta.rs`
- `crates/rocklake-sql/src/classifier/`
...
- `SelectSnapshot`, `SelectTableStats`, `SelectMetadata`, `SelectViews`, `SelectMacros`, `SelectDeleteFiles`, and inlined reads currently return empty result sets in `crates/rocklake-pgwire/src/executor/mod.rs`.
...
- A per-table rowid counter exists (`COUNTER_NEXT_ROWID_PREFIX`) and `rocklake.next_rowid_range` exists, but the DuckLake `next_row_id` column is not present in `TableStatsRow`.
