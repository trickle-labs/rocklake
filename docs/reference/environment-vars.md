...
RockLake v0.48.0 reads the variables below through its typed CLI. Command-line
...
ROCKLAKE_AUTH_PASSWORD=secret \
rocklake serve
```

RockLake does not read a TOML or YAML configuration file.
...
The S3-compatible endpoint and path-style options are CLI-only in v0.48.0:
