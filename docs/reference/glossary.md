...
**Block Cache** — An in-memory cache maintained by SlateDB that stores recently-read SST blocks. Avoids repeated object storage fetches for frequently-accessed data. Its sizing is managed by SlateDB in v0.51.2.
...
published v0.51.2 deployment. Use the PostgreSQL wire sidecar instead.
