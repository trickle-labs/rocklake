...
- C ABI consumers and language bindings
...
Read functions with an `_at` suffix take an exact `snapshot_id`; zero is an
exact snapshot ID. Use the corresponding `_latest` function when the caller
wants the latest committed state. The legacy names remain exact-ID aliases.
