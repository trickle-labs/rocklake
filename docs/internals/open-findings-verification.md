...
**Verification**: Audited `crates/rocklake-catalog/src/writer/mod.rs`.
...
**Verification**: Audited `crates/rocklake-catalog/src/checkpoint.rs`.
...
`crates/rocklake-catalog/src/export.rs`. The function re-scans all
...
`crates/rocklake-datafusion/src/catalog_provider.rs`. It reads the
...
`crates/rocklake-datafusion/benches/datafusion_bridge.rs` records the
before/after improvement.

**Status**: ✅ Closed — persistent thread implemented.

## N-06 — `rocklake-sqlite-vfs` Placeholder

**Finding**: The `rocklake-sqlite-vfs` crate exists in the workspace but
...
**Action taken**: Removed via `git rm -r crates/rocklake-sqlite-vfs`.
...
falls below 80 %. Per-crate minimums are enforced: `rocklake-core` ≥ 85 %,
`rocklake-catalog` ≥ 85 %, `rocklake-sql` ≥ 80 %, `rocklake-pgwire` ≥ 75 %.
`rocklake-sqlite-vfs` (deleted) was removed from the coverage crate list.

**Status**: ✅ Closed — hard gate in CI; `exit 1` on coverage failures.

## N-10 — Missing Doc-Tests for Public APIs

**Finding**: `rocklake-core` and `rocklake-catalog` had no `#![deny(missing_docs)]`
enforcement and no `# Examples` doc-tests on key public functions.

**Fix**:
- Added `#![deny(missing_docs)]` to `rocklake-core/src/lib.rs` and
  `rocklake-catalog/src/lib.rs`.
...
**Fix**: Added `crates/rocklake-pgwire/tests/pgwire_network_test.rs` with
five tests:
...
`cargo test -p rocklake-pgwire --test pgwire_network_test`.
...
**Test**: Added `crates/rocklake-catalog/tests/checkpoint_restore.rs` with:
...
(`rocklake_operations_total`, `rocklake_storage_requests_total`,
`rocklake_sessions_active`, etc.) that are not emitted by the implementation.
...
`rocklake_snapshots_created_total`, `rocklake_files_per_snapshot`,
`rocklake_object_store_requests_total`, `rocklake_object_store_bytes_read_total`,
`rocklake_object_store_bytes_written_total`, `rocklake_object_store_throttles_total`,
`rocklake_object_store_retries_total`, `rocklake_active_sessions`,
`rocklake_max_sessions`, `rocklake_writer_epoch_age_ms`,
`rocklake_last_query_keys_scanned`, `rocklake_cdc_record_count_mismatch_total`.
...
**Fix**: Added `crates/rocklake-catalog/tests/concurrent_writer_fencing.rs`
with three tests:
