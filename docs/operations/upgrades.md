...
The v0.63.3 binary can be upgraded directly from v0.60.0. Stop the current
process and restart it with the same catalog URL and flags. Catalog state
remains in the configured object store; v0.63.3 does not change the catalog
storage format.
...
verifies the catalog before activation. In v0.63.3 the registered migration is
...
The supported mixed-version window is v0.60.0 readers during a v0.63.3 binary
