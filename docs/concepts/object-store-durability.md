...
SlateDB is an LSM-tree (Log-Structured Merge-Tree) key-value engine that maps its internal storage abstractions onto object storage operations. Understanding this mapping is key to understanding RockLake's performance characteristics.
...
When RockLake commits a catalog transaction, durability is established through a sequence of operations:
...
Only after step 4 does DuckDB consider the transaction committed. This means that when DuckDB receives a successful COMMIT response, the catalog mutation is durable in at least two availability zones. A crash of the RockLake process, the host machine, a network switch, or even an entire availability zone cannot lose the mutation.

The object store's eleven-nines durability guarantee kicks in at step 2. From the moment the WAL PUT completes, the data is protected by the cloud provider's cross-AZ replication. RockLake's durability is bounded below by the cloud provider's SLA and above by nothing — there is no weaker link in the chain.
...
## How RockLake Manages Latency

RockLake uses several strategies to minimize the effective per-operation latency seen by DuckDB:
...
When developing and testing RockLake without cloud credentials, you can use the local filesystem backend. RockLake supports a `file://` path prefix that stores all SST files and WAL segments as regular files on disk. This provides the same key-value API and MVCC semantics but with local-filesystem latency. The durability guarantees are different (local filesystem, not cloud object store), but for development purposes this is usually acceptable.
...
- **[SlateDB Storage Engine](slatedb.md)** — A deeper treatment of the LSM-tree engine that sits between RockLake and object storage
...
SlateDB, the LSM-tree engine that RockLake uses for catalog persistence, maps its storage abstractions onto object storage operations:
...
Once RockLake's `commit` returns successfully, the catalog mutation is durable. Specifically:
...
This means RockLake's durability is bounded by your cloud provider's SLA, not by any aspect of the RockLake software. If S3 loses your data, that's an S3 problem, not a RockLake problem.
...
RockLake mitigates this through several strategies:
...
Object storage provides **read-after-write consistency** for new objects (you can read an object immediately after writing it) and **strong consistency** for list operations (a list returns all objects that have been successfully PUT). RockLake relies on both of these guarantees for correct operation.
...
Because RockLake delegates durability to object storage, its failure modes are:

1. **Object storage unavailable:** RockLake cannot read or write. Operations fail with retriable errors. No data is lost. Resume when the outage resolves.
2. **Network partition:** Same as unavailable — RockLake cannot reach storage. Fail, retry, resume.
3. **RockLake process crash:** Catalog state is fully persistent. Restart the process and it resumes from the latest durable state.
...
For the highest durability requirements, you can use cross-region replication features of your cloud provider (S3 Cross-Region Replication, GCS Multi-Region, Azure GRS). RockLake does not need to know about this — it is handled transparently at the storage layer.
