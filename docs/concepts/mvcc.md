...
Multi-Version Concurrency Control is the mechanism that makes SlateDuck's time travel, crash safety, and reader independence work at the implementation level. If immutability is the architectural principle (we never delete committed facts), then MVCC is the technique that makes immutability useful — it is the system that answers the question "given that many versions of each entity coexist in storage, which version should this particular reader see?"

DuckLake's versioning model uses `begin_snapshot` and `end_snapshot` columns on catalog rows to express the snapshot interval during which a given version of a row is visible. SlateDuck maps this model onto SlateDB's key-value layout in a way that makes the most common query patterns efficient. This page explains how the mapping works, what the visibility filter looks like, how different types of catalog entities are versioned, and how garbage collection interacts with the MVCC model.
...
**Snapshot 5: Table created.** DuckDB issues `CREATE TABLE analytics.events (...)`. SlateDuck writes:
...
**Snapshot 12: Table renamed.** DuckDB issues `ALTER TABLE analytics.events RENAME TO user_events`. SlateDuck atomically:
...
**Snapshot 15: Column added.** DuckDB issues `ALTER TABLE analytics.user_events ADD COLUMN region VARCHAR`. SlateDuck writes:
...
Not all DuckLake catalog tables use the full `begin_snapshot`/`end_snapshot` MVCC model. SlateDuck classifies tables into three categories based on their versioning behavior:
...
This distinction is subtle but important for anyone reading the SlateDuck source code or diagnosing MVCC-related issues:

**`dl_snapshot_id`** is the DuckLake-level catalog version — the monotonically increasing integer that identifies a catalog snapshot. It is the value stored in `begin_snapshot` and `end_snapshot` columns, and it is what DuckDB specifies when doing time travel (`ATTACH ... SNAPSHOT '15'`). It is managed by SlateDuck's counter allocation system and advances with every catalog mutation.
...
The dominant cost in any SlateDuck read operation is the object-store I/O to fetch SST blocks — typically 20–50 ms per block on S3 Standard. The MVCC filtering that happens after the data is in memory is orders of magnitude faster than the I/O and does not meaningfully affect operation latency.
