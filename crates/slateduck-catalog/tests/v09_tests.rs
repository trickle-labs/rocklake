...
use slateduck_catalog::cache::cache_utilization;
use slateduck_catalog::corpus::{corpus_diff, corpus_validate, parse_corpus, CorpusRecord};
use slateduck_catalog::cost::{ApiCallSnapshot, ApiCostReport, CostMode};
use slateduck_catalog::migrate::{migrate_apply, migrate_dry_run};
use slateduck_catalog::warmup::{
...
async fn writer_endpoint_publish_and_read() {
    ...
    publish_writer_endpoint(catalog.db(), 1, "pod-a.slateduck.svc.cluster.local:5432")
        .await
        .unwrap();
    ...
    assert_eq!(
        ep.as_deref(),
        Some("pod-a.slateduck.svc.cluster.local:5432")
    );
...
async fn migrate_apply_creates_backup_and_updates_version() {
    ...
    let state = slateduck_catalog::inspect::inspect_snapshot(catalog.db())
        .await
        .unwrap();
...
fn corpus_diff_detects_added_statement_family() {
    ...
    assert_eq!(
        added[0].change_type,
        slateduck_catalog::corpus::DiffChangeType::Added
    );
}
...
fn corpus_diff_detects_removed_statement_family() {
    ...
    assert_eq!(
        ...
        slateduck_catalog::corpus::DiffChangeType::Removed
    );
}
