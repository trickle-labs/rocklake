...
use slateduck_core::tags::{SYSTEM_ENDPOINT, SYSTEM_WRITER_EPOCH};
...
/// This function should be called by the `slateduck warmup` CLI command or
...
pub async fn warmup_cache(db: &Db, max_tables: usize) -> CatalogResult<WarmupResult> {
    ...
    let format_key = keys::key_system(slateduck_core::tags::SYSTEM_CATALOG_FORMAT_VERSION);
...
mod tests {
    ...
    async fn writer_endpoint_roundtrip() {
        ...
        // Publish an endpoint
        publish_writer_endpoint(&db, 1, "pod-a.slateduck.svc:5432")
            .await
            .unwrap();

        let ep = read_writer_endpoint(&db).await.unwrap();
        assert_eq!(ep.as_deref(), Some("pod-a.slateduck.svc:5432"));
