...
fn public_surface_manifest_matches_inventories_and_fixtures() {
    ...
    assert_eq!(manifest["release"], "v0.64.0");
    ...
    assert!(
        snapshot_releases.iter().any(|release| release == "v0.64.0"),
        "compatibility snapshots must include the current release"
    );
    assert!(
        snapshot_releases.iter().any(|release| release == "v0.63.7"),
        "compatibility snapshots must include the previous release"
    );
    assert!(
        snapshot_releases.iter().any(|release| release == "v0.63.5"),
        "compatibility snapshots must preserve the previous release"
    );
    assert!(
        snapshot_releases.iter().any(|release| release == "v0.63.1"),
        "compatibility snapshots must preserve v0.63.1"
    );
    assert!(
        snapshot_releases.iter().any(|release| release == "v0.63.0"),
        "compatibility snapshots must preserve v0.63.0"
    );
    assert!(
        snapshot_releases
            .iter()
            .any(|release| release == "v0.47.17"),
        "compatibility snapshots must include the immediately previous release"
    );
    assert!(
        snapshot_releases
            .iter()
