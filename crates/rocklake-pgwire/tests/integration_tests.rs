...
async fn test_sqlstate_batch_too_large() {
    use rocklake_pgwire::session::{BufferedOp, PendingCatalogTxn};
...
async fn test_wire_handshake_replay() {
    ...
    for line in fixture.lines() {
        ...
        if v["direction"] == "client_to_server" && v["type"] == "Query" {
            ...
            let result = rocklake_sql::classify_statement(query);
            assert!(
                result.is_ok(),
            ...
            assert!(
                !matches!(kind, rocklake_sql::StatementKind::Unsupported(_)),
...
async fn test_pgwire_server_lifecycle() {
    ...
    let server_handle = tokio::spawn(async move {
        rocklake_pgwire::server::run_server_with_shutdown(config, catalog_clone, rx)
            .await
            .unwrap();
    });
...
async fn test_audit_log_write_and_read() {
    ...
    let opts = rocklake_catalog::OpenOptions {
        object_store,
        path: ObjectPath::from(""),
        encryption: None,
    };
    let catalog = rocklake_catalog::CatalogStore::open(opts).await.unwrap();

    let entry = rocklake_catalog::AuditEntry {
        snapshot_id: 1,
        committed_at: "2025-05-23T12:00:00Z".to_string(),
        committed_by: "pg_tide".to_string(),
        changes: vec![rocklake_catalog::AuditChange {
            change_type: "register_data_file".to_string(),
            detail: Some("data/orders/part-00042.parquet".to_string()),
        }],
    };

    rocklake_catalog::audit::write_audit_entry(catalog.db(), &entry)
        .await
        .unwrap();

    let entries = rocklake_catalog::audit::list_audit_entries(catalog.db())
        .await
        .unwrap();
    ...
    // Read specific entry
    let specific = rocklake_catalog::audit::get_audit_entry(catalog.db(), 1)
        .await
        .unwrap();
    ...
    // Non-existent entry
    let missing = rocklake_catalog::audit::get_audit_entry(catalog.db(), 999)
        .await
        .unwrap();
...
#[test]
fn test_tls_config() {
    use rocklake_pgwire::server::TlsConfig;
...
#[test]
fn test_auth_config() {
    use rocklake_pgwire::server::AuthConfig;
...
async fn test_tls_required_without_certs_fails_start() {
    ...
    let (tx, rx) = tokio::sync::oneshot::channel::<()>();
    let result = rocklake_pgwire::server::run_server_with_shutdown(config, catalog, rx).await;
    assert!(
        result.is_err(),
...
fn test_spark_corpus_all_statements_classifiable() {
    ...
    for stmt in statements {
        ...
        let result = rocklake_sql::classify_statement(sql);
        assert!(
            result.is_ok(),
...
async fn test_auth_no_auth_configured_any_user_succeeds() {
    let dir = tempfile::tempdir().unwrap();
    let auth = rocklake_pgwire::server::AuthConfig::default(); // no auth
    let (addr, tx, handle) = start_server_with_auth(&dir, auth).await;
...
/// `SELECT * FROM rocklake_catalog.ducklake_snapshot` returns a row.
#[tokio::test]
...
/// `SELECT * FROM rocklake_catalog.ducklake_schema` returns schemas.
#[tokio::test]
async fn test_virtual_catalog_schema() {
    ...
    let responses = executor::execute_sql(
        "SELECT * FROM rocklake_catalog.ducklake_schema",
        &params,
        &store,
        &mut session,
        &default_notify_manager(),
...
/// `SELECT * FROM rocklake_catalog.ducklake_table` returns all tables.
#[tokio::test]
async fn test_virtual_catalog_table() {
    ...
    let responses = executor::execute_sql(
        "SELECT * FROM rocklake_catalog.ducklake_table",
        &params,
        &store,
        &mut session,
        &default_notify_manager(),
...
/// `SELECT * FROM rocklake_catalog.rocklake_counters` returns a result.
#[tokio::test]
async fn test_virtual_catalog_counters() {
    ...
    let responses = executor::execute_sql(
        "SELECT * FROM rocklake_catalog.rocklake_counters",
        &params,
        &store,
        &mut session,
        &default_notify_manager(),
...
/// Operator introspection: a WHERE clause on rocklake_catalog.ducklake_data_file is classifiable.
#[tokio::test]
async fn test_virtual_catalog_data_file_classifiable() {
    use rocklake_sql::classify_statement;
    use rocklake_sql::StatementKind;

    let sql = "SELECT data_file_id, path, begin_snapshot FROM rocklake_catalog.ducklake_data_file WHERE table_id = 42 ORDER BY begin_snapshot DESC LIMIT 20";
