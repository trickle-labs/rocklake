...
/// Verify that a freshly initialised SlateDuck catalog has readable metadata
...
async fn view_lifecycle_via_duckdb() {
    ...
    // The full lifecycle test would require a running SlateDuck PgWire server;
...
fn registry_includes_encryption_key() {
    use slateduck_pgwire::schema_registry::fields_for_table;
