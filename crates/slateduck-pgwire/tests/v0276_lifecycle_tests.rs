...
//!
//! Exercises the full DuckDB/DuckLake lifecycle against a live SlateDuck
...
//!
//! ```sh
//! cargo test -p slateduck-pgwire --test v0276_lifecycle_tests -- --test-threads=1
//! ```
//!
...
/// the raw `ducklake_inlined_data_*` rows from SlateDuck and verify the
    ...
    let sql = format!(
        "LOAD ducklake; \
         ...
         SELECT * FROM postgres_query( \
             'host=127.0.0.1 port={port} dbname=slateduck', \
...
// They complement the unit tests in slateduck-catalog/src/writer/stats.rs.
