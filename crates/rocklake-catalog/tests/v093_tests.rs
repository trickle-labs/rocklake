...
async fn read_at_below_retain_from_returns_error() {
    ...
    rocklake_catalog::gc::gc_apply(&db, 5).await.unwrap();
...
async fn excise_apply_fails_when_retain_from_is_zero() {
    ...
    let result = rocklake_catalog::excise::excise_apply(&db, 1, "operator").await;
    assert!(
        result.is_err(),
...
async fn checkpoint_restore_hides_post_checkpoint_facts() {
    ...
    let db = open_db(&dir).await;
    let cp = rocklake_catalog::checkpoint::create_checkpoint(&db, Some("cp1"))
        .await
        .unwrap();
    db.close().await.unwrap();
...
fn nan_comparison_is_fail_closed() {
    use rocklake_core::types::{DuckLakeType, PruneResult, TypeCompareError};

    let float_type = DuckLakeType::Float { width_bits: 64 };

    // NaN min or max comparison → error from type_aware_compare
    let result = rocklake_core::types::type_aware_compare("5.0", "NaN", &float_type);
    assert_eq!(
        result,
    ...
    let prune =
        rocklake_core::types::prune_file("5.0", Some("NaN"), Some("10.0"), false, &float_type);
    assert_eq!(
        prune,
        Ok(PruneResult::Keep),
