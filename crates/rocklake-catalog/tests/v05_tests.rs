...
#[test]
fn ffi_abi_version() {
    assert_eq!(rocklake_ffi::rocklake_abi_version(), 5_000);
}
...
fn ffi_open_close_roundtrip() {
    ...
    let snap = rocklake_ffi::rocklake_get_current_snapshot(catalog, &mut err);
    assert_eq!(err.code, 0);
    assert_eq!(snap.snapshot_id, 0);

    rocklake_ffi::rocklake_close(catalog);
}
...
async fn strategy_b_c_equivalence() {
    ...
    let (ffi_schema_id, ffi_table_name, ffi_col_count, ffi_file_id_result, ffi_snap_id) =
        tokio::task::spawn_blocking(move || {
            ...
            // List tables
            let tables = rocklake_ffi::rocklake_list_tables(catalog, ffi_sid, 1, &mut err);
            assert_eq!(err.code, 0);
...
async fn all_28_ducklake_tables_tested() {
    use rocklake_core::tags::{TagStatus, ALL_TAGS};
