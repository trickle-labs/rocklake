...
async fn writer_epoch_missing_key_fails_closed() {
    ...
    let epoch_key = rocklake_core::keys::key_system(rocklake_core::tags::SYSTEM_WRITER_EPOCH);
...
async fn extension_concurrent_inserts_unique_ids() {
    ...
    rocklake_catalog::extension::create_extension_table(
        db,
        rocklake_catalog::extension::EXTENSION_PGTRICKLE,
        "cursors",
    )
    .await
    .unwrap();
    ...
    let mut ids = Vec::new();
    for i in 0..10 {
        let id = rocklake_catalog::extension::insert_extension_row(
            db,
            rocklake_catalog::extension::EXTENSION_PGTRICKLE,
            "cursors",
            &format!("{{\"cursor\":{i}}}"),
...
async fn gc_advance_respects_lease() {
    ...
    rocklake_catalog::lease::hold_snapshot(db, "test-consumer", 3, 3600)
        .await
        .unwrap();

    // Try to advance retain-from past the leased snapshot
    let result = rocklake_catalog::gc::gc_apply(db, 4).await;
    assert!(
        result.is_err(),
...
async fn rowid_range_rejects_zero_count() {
    ...
    let result = rocklake_catalog::next_rowid_range(store.db(), 1, 0).await;
...
async fn rowid_range_overflow_detection() {
    ...
    let (_, _) = rocklake_catalog::next_rowid_range(db, 99, u64::MAX - 10)
        .await
        .unwrap();

    // Now try to allocate more than fits
    let result = rocklake_catalog::next_rowid_range(db, 99, 20).await;
...
async fn lease_ttl_overflow_detected() {
    ...
    let result =
        rocklake_catalog::lease::hold_snapshot(db, "overflow-consumer", 1, u64::MAX / 500).await;
...
fn table_changes_stream_reconstructs_end_state() {
    use rocklake_sql::table_changes::*;
