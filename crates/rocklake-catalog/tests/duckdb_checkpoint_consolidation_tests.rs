...
async fn test_checkpoint_consolidation_no_duplication() {
    ...
    eprintln!(
        "✓ Phase 1: Registered file1 (id={}, 2 rows) at snapshot {}",
        file1_id, snap2.snapshot_id
    );
    ...
    let file2_id = writer
        ...
        .unwrap();
    eprintln!(
        "✓ Phase 5b: Registered file2 (id={}, 2 rows) as consolidated version",
        file2_id
    );
    ...
    // Only file2 is visible
    {
        ...
        for f in &files {
            eprintln!(
                "  file_id={}, begin_snapshot={:?}, end_snapshot={:?}, rows={}",
        ...
        assert_eq!(
            files[0].record_count, 2,
            "Consolidated file should still have 2 rows"
        );

        // Verify the old file is not visible (has end_snapshot)
        assert!(
            files[0].end_snapshot.is_none(),
            "Active file should not have end_snapshot set"
        );
...
async fn test_consolidation_same_snapshot_duplication() {
    ...
    store.commit_writer(snap2.clone());
    eprintln!(
        "Phase 2: Registered file1 id={} at snapshot {}",
        file1_id, snap2.snapshot_id
    );
    ...
    let file2_id = writer
        .register_data_file(
            table_id,
            "s3://bucket/consolidated.parquet",
            "parquet",
            2,
            1024,
        )
    ...
    store.commit_writer(snap3.clone());
    eprintln!(
        "Phase 3: Deleted file1, registered file2 id={} at snapshot {}",
        file2_id, snap3.snapshot_id
    );
    ...
    // MVCC filtering based on begin_snapshot/end_snapshot ensures file1 is excluded
    {
        ...
        let files = reader.list_data_files(table_id).await.unwrap();
        eprintln!(
            "At snapshot {}: {} visible files",
            snap3.snapshot_id,
            files.len()
        );
        for f in &files {
            eprintln!(
                "  file_id={}, begin={:?}, end={:?}, records={}",
                f.data_file_id, f.begin_snapshot, f.end_snapshot, f.record_count
            );
        }
        ...
        assert_eq!(
            files.len(),
            1,
            "At snapshot {}: expected 1 file but got {}",
            snap3.snapshot_id,
            files.len()
        );
        assert_eq!(files[0].data_file_id, file2_id);
...
async fn test_multiple_checkpoint_cycles_no_duplication() {
    ...
    // Verify after cycle 1
    {
        ...
        eprintln!(
            "Cycle 1 verified: {} files, {} total rows",
            files.len(),
            total
        );
        assert_eq!(
            total, expected_total_rows,
            "Should have {} rows after cycle 1",
            expected_total_rows
        );
    ...
    let file2_id = {
        ...
        store.commit_writer(snap.clone());
        eprintln!(
            "Cycle 2: CHECKPOINT completed, registered consolidated file (id={}) with {} rows",
            fid, expected_total_rows
        );
        fid
    };
    ...
    // Verify after cycle 2: Should NOT duplicate
    {
        ...
        let total: u64 = files.iter().map(|f| f.record_count).sum();
        eprintln!(
            "Cycle 2 verified: {} files, {} total rows",
            files.len(),
            total
        );
        ...
        assert_eq!(
            files.len(),
            1,
            "After cycle 2: should have only 1 file (consolidated)"
        );
        assert_eq!(
            files[0].data_file_id, file2_id,
            "Should see consolidated file2"
        );
