...
async fn gc_plan_shows_zero_on_fresh_catalog() {
    ...
    let plan = rocklake_catalog::gc::gc_plan(&db, 30).await.unwrap();
...
async fn gc_apply_advances_retain_from() {
    ...
    let result = rocklake_catalog::gc::gc_apply(&db, 2).await.unwrap();
...
async fn gc_respects_pinned_snapshots() {
    ...
    // Pin snapshot 1
    rocklake_catalog::gc::pin_snapshot(&db, 1).await.unwrap();

    // Attempting to advance past pinned snapshot should fail
    let result = rocklake_catalog::gc::gc_apply(&db, 2).await;
    assert!(result.is_err());

    // Unpin and retry
    rocklake_catalog::gc::unpin_snapshot(&db, 1).await.unwrap();
    let result = rocklake_catalog::gc::gc_apply(&db, 2).await.unwrap();
    assert_eq!(result.new_retain_from, 2);
...
async fn excise_apply_deletes_and_records_audit() {
    ...
    rocklake_catalog::gc::gc_apply(&db, 2).await.unwrap();

    // Apply excision
    let result = rocklake_catalog::excise::excise_apply(&db, 2, "test-operator")
        .await
        .unwrap();
    ...
    let audits = rocklake_catalog::excise::read_audit_entries(&db)
        .await
        .unwrap();
...
async fn excise_refuses_without_retain_from_advance() {
    ...
    rocklake_catalog::gc::gc_apply(&db, 1).await.unwrap();

    // Trying to excise at 5 (beyond retain-from=1) should fail
    let result = rocklake_catalog::excise::excise_apply(&db, 5, "test").await;
    assert!(result.is_err());
...
async fn checkpoint_create_list_restore() {
    ...
    // Create checkpoint
    let cp = rocklake_catalog::checkpoint::create_checkpoint(&db, Some("test-backup"))
        .await
        .unwrap();
    ...
    // List checkpoints
    let list = rocklake_catalog::checkpoint::list_checkpoints(&db)
        .await
        .unwrap();
    ...
    // Restore checkpoint
    let restored = rocklake_catalog::checkpoint::restore_checkpoint(&db, cp.id)
        .await
        .unwrap();
...
async fn checkpoint_restore_after_modifications() {
    ...
    let cp = rocklake_catalog::checkpoint::create_checkpoint(&db, Some("before-changes"))
        .await
        .unwrap();
    assert_eq!(cp.snapshot_id, 1);
...
async fn export_import_roundtrip() {
    ...
    let result = rocklake_catalog::export::export_catalog(&db, None, &mut export_buf)
        .await
        .unwrap();
    ...
    let import_result = rocklake_catalog::export::import_catalog(&db2, reader)
        .await
        .unwrap();
...
async fn pg_migrate_produces_insert_statements() {
    ...
    let count = rocklake_catalog::export::pg_migrate(reader, &mut output).unwrap();
    assert!(count > 0);
...
async fn rebuild_from_file_paths() {
    ...
    let result = rocklake_catalog::inspect::inspect_snapshot(&db)
        .await
        .unwrap();
...
async fn verify_data_files_reports_missing() {
    ...
    let db = open_db(&dir).await;
    let result = rocklake_catalog::cleanup::verify_data_files(&db, &object_store)
        .await
        .unwrap();
...
async fn repair_plan_on_healthy_catalog_empty() {
    ...
    let db = open_db(&dir).await;
    let plan = rocklake_catalog::repair::repair_plan(&db).await.unwrap();
...
async fn repair_fixes_stale_counter() {
    ...
    // Apply repair
    let result = rocklake_catalog::repair::repair_apply(&db, &plan)
        .await
        .unwrap();
...
async fn metrics_render_prometheus_format() {
    let metrics = rocklake_catalog::CatalogMetrics::new(50);
    ...
    assert!(output.contains("rocklake_snapshots_created_total 2"));
    assert!(output.contains("rocklake_files_per_snapshot 10"));
    assert!(output.contains("rocklake_object_store_requests_total 1"));
    assert!(output.contains("rocklake_max_sessions 50"));
}
...
fn encryption_config_from_valid_hex() {
    ...
    let config = rocklake_catalog::encryption::EncryptionConfig::from_hex(hex).unwrap();
...
fn encryption_config_rejects_short_key() {
    ...
    let result = rocklake_catalog::encryption::EncryptionConfig::from_hex(hex);
    assert!(result.is_err());
}
...
async fn collect_referenced_paths_includes_all_files() {
    ...
    let db = open_db(&dir).await;
    let paths = rocklake_catalog::cleanup::collect_referenced_paths(&db)
        .await
        .unwrap();
