...
impl CatalogWriter {
    ...
    async fn allocate_row_id_start(
        ...
        if let Some((_, val)) = self.staged.iter().find(|(k, _)| k == &key) {
            if let Ok(row) =
                rocklake_core::values::decode_value::<rocklake_core::rows::TableStatsRow>(val)
            {
        ...
        if existing_stats.is_none() {
            if let Some(data) = self.db.get(&key).await? {
                if let Ok(row) =
                    rocklake_core::values::decode_value::<rocklake_core::rows::TableStatsRow>(&data)
                {
                    row_id_start = row.next_row_id.unwrap_or(0);
                    existing_stats = Some(row);
                }
        ...
        let prev_file_size_bytes = existing_stats
            .as_ref()
            .map(|r| r.file_size_bytes)
            .unwrap_or(0);
        let prev_internal_file_count = existing_stats
            .as_ref()
            .map(|r| r.internal_file_count)
            .unwrap_or(0);
    ...
    pub async fn drop_table(
        ...
        {
            let prefix = keys::prefix_for_tag(rocklake_core::tags::TAG_PARTITION_INFO);
            let mut iter = self
                .db
                .scan_prefix(&prefix)
                .await
        ...
        {
            let prefix = keys::prefix_for_tag(rocklake_core::tags::TAG_COLUMN_TAG);
            let mut iter = self
                .db
                .scan_prefix(&prefix)
                .await
        ...
        {
            let prefix = keys::prefix_for_tag(rocklake_core::tags::TAG_SORT_INFO);
            let mut iter = self
                .db
                .scan_prefix(&prefix)
                .await
    ...
    pub async fn find_table_begin_snapshot(
        &self,
        schema_id: u64,
        table_id: u64,
    ...
    pub async fn find_column_begin_snapshot(
        &self,
        table_id: u64,
        column_id: u64,
    ) -> CatalogResult<Option<u64>> {
    ...
    pub async fn register_data_file(
        ...
        let row_id_start = self
            .allocate_row_id_start(table_id, record_count, file_size_bytes)
            .await?;
    ...
    pub fn set_metadata(
        ...
        let scope_str = match scope {
            rocklake_core::keys::MetadataScope::Global => "global",
            rocklake_core::keys::MetadataScope::Schema => "schema",
            rocklake_core::keys::MetadataScope::Table => "table",
        };
        let row = MetadataRow {
            key: key.to_string(),
